...
export default function App() {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          return parsed.map((item: any, idx: number) => {
            ...
            return {
              ...
              apartment: item.apartment ?? fallback.apartment ?? 'Apartment A',
              isActive: item.isActive ?? fallback.isActive ?? true,
              teamMembers: item.teamMembers ?? fallback.teamMembers ?? [],
              machineCount: item.machineCount ?? fallback.machineCount ?? 40
            };
          });
        }
      }
  ...
  // WIP Level & Buffer Monitoring Notification Trigger
  // Monitors in-line WIP levels across all lines and flags an alert in NotificationsModal
  // if WIP exceeds the calculated threshold for a specific style.
  useEffect(() => {
      ...
      <main className="flex-1 max-w-[1500px] w-full mx-auto px-3 sm:px-6 py-5 sm:py-7">
        {currentTab === 'dashboard' && (
          <Dashboard
            ...
            onChecklistCompleted={() => handleUpdateChecklistTask(todayStr, 0, 'yes')}
